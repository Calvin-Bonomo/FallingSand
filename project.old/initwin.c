...
//
GLFWwindow* InitWindow(const char* title,int sync,int width,int height , void (*reshape)(GLFWwindow*,int,int) , void (*key)(GLFWwindow*,int,int,int,int), void (*mouse)(GLFWwindow*,int,int,int), void (*move)(GLFWwindow*,double,double))
{
   //  Initialize GLFW
   ...
   if (mouse) glfwSetMouseButtonCallback(window, mouse);
   if (move) glfwSetCursorPosCallback(window, move);
